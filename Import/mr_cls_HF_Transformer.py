...
class MR_transformer:
    ...
    def mr_load_model(self, m_path):   
        ...
        self.mr_c =  TFDistilBertForSequenceClassification.from_pretrained(m_path)
    ...
    def mr_score_data(self,score_df):
        ...
        # Preproceess the scoring
        score_data = self.mr_t(score_df["Answer"].tolist(), truncation=True, padding = True) 
        score_dataset = tf.data.Dataset.from_tensor_slices((
                dict(score_data)
                ))
        
        # Get the predictions for the input
        y_pred = self.mr_c.predict(score_dataset.batch(1))
        y_pred = np.argmax(y_pred.logits, axis=-1).tolist()
